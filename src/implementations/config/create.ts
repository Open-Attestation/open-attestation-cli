...
export const create = async ({
  ...
  const networkId = getNetworkId(configNetwork);
  ...
  if (hasTransferableRecord) {
    tokenRegistryAddress = await getTokenRegistryAddress(encryptedWalletPath, configNetwork);
  ...
  if (hasDocumentStore) {
    documentStoreAddress = await getDocumentStoreAddress(encryptedWalletPath, configNetwork);
  ...
  const updatedConfigFileWithNetwork = getConfigWithUpdatedNetwork({ configFile, network: configNetwork });
