...
import { connectToTitleEscrow, validateTransferBeneficiary } from "./helpers";
...
export const endorseNominatedBeneficiary = async ({
  ...
  await validateTransferBeneficiary({ to: nominatedBeneficiary, titleEscrow, walletAddress });
  if (dryRun) {
    await dryRunMode({
