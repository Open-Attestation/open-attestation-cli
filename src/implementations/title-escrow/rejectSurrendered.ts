import { TitleEscrowCloneableFactory, TradeTrustERC721Factory, TradeTrustERC721 } from "@govtechsg/token-registry";
...
const retrieveLastBeneficiaryAndHolder = async (
  ...
  const lastTitleEscrowInstance = await TitleEscrowCloneableFactory.connect(lastTitleEscrowAddress, wallet);
...
export const rejectSurrendered = async ({
  ...
  if (dryRun) {
    ...
    await dryRunMode({
      ...
      estimatedGas: await tokenRegistryInstance.estimateGas.restoreTitle(lastBeneficiary, lastHolder, tokenId),
      network,
    });
    process.exit(0);
  }
  ...
  await tokenRegistryInstance.callStatic.restoreTitle(lastBeneficiary, lastHolder, tokenId, {
    gasPrice: gasPrice.mul(gasPriceScale),
  });
  const transaction = await tokenRegistryInstance.restoreTitle(lastBeneficiary, lastHolder, tokenId, {
    gasPrice: gasPrice.mul(gasPriceScale),
  });
