...
const { trace } = getLogger("token-registry:grant-role");

export const grantRoleToTokenRegistry = async ({
  address,
  recipient,
  role,
  network,
  ...
  if (dryRun) {
    await dryRunMode({
      ...
      estimatedGas: await tokenRegistry.estimateGas.grantRole(role, recipient),
      network,
    });
    process.exit(0);
  }
  ...
  await tokenRegistry.callStatic.grantRole(role, recipient, {
