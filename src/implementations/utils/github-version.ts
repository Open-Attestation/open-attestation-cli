...
export const getLatestRelease = async (): Promise<GithubReleaseSchema> => {
  return await request(ReleaseAPI);
};

export const getReleaseAsset = async (
  machinePlatform: string,
  availableAssets: GithubAssetsSchema[],
  latest: string
): Promise<GithubDownloadInfo | undefined> => {
...
export const downloadFile = async (downloadInfo: GithubDownloadInfo, path: string): Promise<void> => {
